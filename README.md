...
- **원소 테마 (10스테이지 블록마다 순환).** 11스테이지부터 블록마다 테마가 바뀌며(불 → 얼음 → 번개) 필드에 해저드 패치가 깔립니다. 그리는 중에 밟으면 — **불 🔥**: 긋던 절개선이 통째로 타버리고 시작점으로 복귀(불은 절대 뚫을 수 없음 — 감싸서 정화하세요), **얼음 ❄**: 3초간 이동속도 절반, **번개 ⚡**: 1초간 완전 정지. 해저드가 깔린 땅을 영역으로 감싸면 정화되어 사라집니다.
